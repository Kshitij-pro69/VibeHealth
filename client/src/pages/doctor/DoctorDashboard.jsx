...
import { Calendar, Clock, User, Sparkles, CheckCircle, FileText, AlertTriangle } from 'lucide-react';
            ...
            {appointments.map((apt) => (
              <Card key={apt._id} className="p-6 space-y-4">
                ...
                <div className="text-xs text-slate-700 bg-slate-50 p-3 rounded-xl">
                  ...
                  {apt.reasonForVisit}
                  {apt.patientNotes && <p className="mt-1 text-slate-600 italic">"{apt.patientNotes}"</p>}
                </div>

                {/* AI Pre-Visit Triage Assistance Box */}
                {apt.preVisitSummary?.symptoms?.length > 0 && (
                  <div className="p-4 bg-teal-50/70 border border-teal-200/70 rounded-2xl space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2 text-teal-800 font-bold text-xs">
                        <Sparkles className="w-4 h-4 text-teal-600" />
                        <span>Clinician-Reference Triage Assistance (AI Generated)</span>
                      </div>
                      <Badge
                        variant={
                          apt.preVisitSummary.severity === 'high' || apt.preVisitSummary.severity === 'emergency'
                            ? 'danger'
                            : 'primary'
                        }
                      >
                        {apt.preVisitSummary.severity.toUpperCase()} PRIORITY
                      </Badge>
                    </div>

                    <p className="text-xs text-slate-700">
                      <strong>Symptoms Extracted:</strong> {apt.preVisitSummary.symptoms.join(', ')}
                    </p>

                    {apt.preVisitSummary.triageNotes && (
                      <p className="text-xs text-slate-600 bg-white/80 p-2.5 rounded-xl border border-teal-100">
                        {apt.preVisitSummary.triageNotes}
                      </p>
                    )}

                    <p className="text-[10px] text-teal-700 italic">
                      ⚠️ Note: For clinician reference only. Not authoritative medical advice.
                    </p>
                  </div>
                )}
              </Card>
